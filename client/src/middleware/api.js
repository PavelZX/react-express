...
import { API_CALL, GET } from '../redux/constants/api_types';
import { UNKNOWN, FETCHING_FAILURE, FETCHING_SUCCESS } from '../redux/constants/action_types';
...
function callApi(options) {
  ...
  return fetch('/api/v1' + path, params)
    .then(response =>
...
export default store => next => action => {
  ...
  return callApi(apiCall).then(
    ...
    response => {
      ...
      let errorMsg = error.message || 'Случилось что-то плохое'
      next(actionWith({
        type: type + '_FAILURE',
        error: errorMsg,
        response
